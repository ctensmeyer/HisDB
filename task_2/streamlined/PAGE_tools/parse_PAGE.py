...
def process_region(region, namespace, find_subregions=False):
    ...
    region_out['bounding_poly'] = extract_points(coords.attrib.get('points', ""))
...
def process_line(line, namespace):
    ...
    line_out['bounding_poly'] = extract_points(coords.attrib.get('points', ""))

    gt_el = line.find(namespace+'TextEquiv')
    ground_truth = None
    if gt_el is not None:
        ground_truth = gt_el.find(namespace+'Unicode').text
